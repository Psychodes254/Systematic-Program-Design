...
(check-expect (find-blank (cons 1 (cons 2 (rest BD1)))) 2)
(check-expect (find-blank (cons 1 (rest BD1))) 1)

(define (find-blank bd)
  (cond [(empty? bd) (error "There is no blank square")]
        [else
         (if (false? (first bd))
             0
...
;; Produce list containing only the valid boards
...
(check-expect (valid-board? (fill-square BD5 1 5)) false)

(define (valid-board? bd)
  (local [(define (valid-units? lou)                                  ;(listof Unit) -> Boolean
            (cond [(empty? lou) true]                                 ;Produce true if every unit in the list is valid
                  [else (andmap valid-unit? lou)]))

          (define (valid-unit? u)                                     ;Unit -> Boolean
            (no-duplicates?                                           ;Produce true if unit has no duplicate value
             (keep-values
             (read-unit u))))
          ...
          (define (read-pos p)                                        ;Pos -> (listof Vale/false)
            (read-square bd p))                                       ;Produce contents of BD at a position

          (define (keep-values lov)                                   ;(listof Val/false) -> (listof Val)
            (filter number? lov))

          (define (no-duplicates? lov)                                ;(listof Value) -> Boolean
            (cond [(empty? lov) true]                                 ;Produce true if no value appears more than once (ignoring false)
                  [(false? (first lov)) (no-duplicates? (rest lov))]
